...
Windows、Node.js 24、PowerShell 7，先執行 `npm ci`、`npm run build -w @omw/manager`，並確認已安裝的 `@sevenflanks/omw` 是 0.2.1。當時已安裝版與本分支的 `Inspect` 邏輯相同（差別在其他 helper action）；以下以該安裝版作基線。從工作樹根目錄執行：
