...
- OMW 與 OpenCode 使用分離的 Basic credentials。launcher token 是第三組 credential，供已實作的 local reserve/register endpoints 使用，且永遠不能授權 browser routes。
...
`npm run credentials:setup -w @omw/manager` 是 interactive local TTY command。它要求 user 以 masked input 設定 OMW 與 OpenCode passwords，建立 random launcher token 但不顯示 token，並只將 Windows current-user DPAPI ciphertext 寫入 `<OMW_DATA_DIR>/credentials.dpapi`。Plaintext 透過 stdin 進入 PowerShell helper，不進 process argv。Ciphertext 以同一 directory 的 temporary file 與 rename 替換。OMW 不修改 Windows ACLs。
...
OpenCode 只在 child environment 收到 `OPENCODE_SERVER_USERNAME` 與 `OPENCODE_SERVER_PASSWORD`。Manager internal OpenCode API calls 使用對應的 Basic header。解密後的 values 留在 backend memory，不由 API 回傳、不寫入 SQLite、不嵌入 URLs，也不刻意寫入 log。

## 設定形式

以下是 configuration shape，不是 production setup command。本文件不執行 Tailscale command；`<device>`、`<tailnet>` 與 credential values 都是 placeholders。
...
`OMW_PORT` 的 server default 是 `4174`；上例的 `4310` 是需明示設定的 example，不是 default。`OMW_REMOTE_MAPPING_READY=1` 只能在 operator 實際核對 mappings 後設定。
...
再次執行 interactive setup 以替換 DPAPI file，接著 stop 並 restart OMW 與每個既有 OpenCode instance。OpenCode 在 process start 從 environment 讀取 Basic credentials，不會 hot-reload 此契約。

只替換 credential file 不會 revoke 已在執行的 OpenCode processes 所持有的 credentials。Basic auth 也沒有可在 server side invalidate 的 browser session；browser 可能會將 credentials cache 在 origin/realm，直到 browser context 關閉或清除 credential cache。在 old processes/endpoints 停止且 clients 重新測試前，不得宣稱已完成 revocation。

## PoC 證據

PoC 只使用 loopback listeners 與 temporary random/fake credentials，不接觸 Tailnet。不可把此 PoC 說成 real Tailscale 驗證。
...
| Auth negative | PASS | 沒有 OpenCode Basic auth 的 `/global/health` 回傳 `401`。 |
...
| Two-instance isolation | PASS | 不同 fake credentials 的兩個 ports 拒絕 cross-used credentials，只回傳各自 backend identity。 |
